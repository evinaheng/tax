	...
	for index, version := range versionsString {
		...
		withParamHandler := func(w http.ResponseWriter, r *http.Request, ps httprouter.Params) {
			...
			*/
			...
			ctx = ctxs.GetAllContextFromRequest(r)
		...
		if result.GetContextError() != nil {
			...
			if result.GetContextError().Error() == ctxs.ContextCancelledError {
				...
				responseJSON = router.Data{
					Errors: router.NewError("97", http.StatusBadRequest, locale.Translate("id-ID", "ErrorServer")),
				}
				...
				// Timeout
				responseJSON = router.Data{
					Errors: router.NewError("14", http.StatusGatewayTimeout, locale.Translate("id-ID", "ErrorServer")),
				}
			...
			// Panics
			responseJSON = router.Data{
				Errors: router.NewError("99", http.StatusInternalServerError, locale.Translate("id-ID", "ErrorServer")),
			}
